...
- カテゴリは **家・買い物・子ども・その他** の4種類です。追加フォーム・編集画面・ドラッグ移動先で共通です。
...
`python3 -m http.server 8000` をリポジトリのルートで起動し、`http://localhost:8000/tools/family-tasks/` を開きます。前回のプロトタイプ作成時、ChromiumのDOM操作と幅320・375・768・1280pxのエミュレーションで追加・編集・完了・削除・JSON入出力・保存復元・境界入力等を確認しています。ドラッグ更新では同じ4画面幅に加えて担当・カテゴリ・期限のドラッグ、スマホ長押し、メニュー操作、既存操作の回帰テスト（16件＋タッチ2件）に合格しました。カテゴリ4種類への変更ではこれらを再実行し、旧6カテゴリの読込・保存・JSON往復・不正カテゴリ拒否も確認しました。ローカルHTTP URLへのブラウザ直接ナビゲーション、iPhone実機、Firebase認証・アクセス権・複数端末同期は未検証です。
