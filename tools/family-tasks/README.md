...
`python3 -m http.server 8000` をリポジトリのルートで起動し、`http://localhost:8000/tools/family-tasks/` を開きます。前回のプロトタイプ作成時、ChromiumのDOM操作と幅320・375・768・1280pxのエミュレーションで追加・編集・完了・削除・JSON入出力・保存復元・境界入力等を確認しています。今回のドラッグ更新では、同じ4画面幅に加えて担当・カテゴリ・期限のドラッグ、スマホ長押し、メニュー操作、既存操作の回帰テスト（16件＋タッチ2件）に合格しました。ローカルHTTP URLへのブラウザ直接ナビゲーション、iPhone実機、Firebase認証・アクセス権・複数端末同期は未検証です。
