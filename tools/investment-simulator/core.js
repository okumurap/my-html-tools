const SimulatorCore = (() => {
  ...
  const definitions=[
    ...
    {id:'split-months',min:1,max:36,step:1,defaultValue:12},
    {id:'crash-year',min:1,max:50,step:1,defaultValue:5},
    {id:'crash-loss',min:0,max:70,step:5,defaultValue:30},
    {id:'crash-recovery',min:0,max:20,step:1,defaultValue:3},
    {id:'goal-amount',min:100,max:100000,step:100,defaultValue:10000},
    {id:'inflation-rate',min:0,max:10,step:0.5,defaultValue:2},
    {id:'current-age',min:20,max:70,step:1,defaultValue:40},
    {id:'end-age',min:60,max:120,step:1,defaultValue:90},
    {id:'withdraw-monthly',min:0,max:100,step:1,defaultValue:20},
    {id:'withdraw-rate',min:-5,max:15,step:0.5,defaultValue:3}
  ];
  const $=id=>document.getElementById(id);
  const format=value=>Math.round(value).toLocaleString('ja-JP')+'万円';
  const signed=value=>{const rounded=Math.round(value);return (rounded>0?'+':'')+rounded.toLocaleString('ja-JP')+'万円';};
  ...
  function draw(id,points,series){
    ...
    const labels=[];
    for(let year=firstYear;year<=finalYear;year+=5)labels.push(year);
  ...
  function setDelta(id,value){
    ...
    el.className='delta'+(Math.round(value)>0?' positive':Math.round(value)<0?' negative':'');
  }

  return {definitions,$,format,signed,clamp,read,simulate,draw,renderRows,setDelta};
})();
