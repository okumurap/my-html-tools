(() => {
  ...
  $('.speed-chart-wrap', speedMap)?.remove();
  const chart = document.createElement('div');
  chart.className = 'speed-chart-wrap';
  chart.innerHTML = '<div class="speed-chart-head"><div class="speed-chart-legend"><span class="speed-chart-key">実速度</span><span class="speed-chart-key limit">制限速度</span></div><strong class="speed-chart-now">0 / 35 km/h</strong></div><canvas class="speed-chart-canvas" aria-label="路線距離ごとの実速度、制限速度、駅位置グラフ"></canvas>';
  ...
  function draw() {
    ...
    const pad = { l: 32 * dpr, r: 10 * dpr, t: 23 * dpr, b: 20 * dpr };
    ...
    ctx.lineWidth = dpr;
    ctx.fillStyle = 'rgba(230,240,245,.75)';
    ...
    [0, 20, 40, 60, 80].forEach(v => {
      ...
      ctx.stroke();
      ctx.fillText(String(v), pad.l - 5 * dpr, y(v));
    });
    ...
    ctx.font = `700 ${7.5 * dpr}px sans-serif`;
    ctx.textBaseline = 'top';
    stations.forEach((station, index) => {
      const sx = x(station.pos);
      ctx.beginPath();
      ctx.moveTo(sx, pad.t);
      ctx.lineTo(sx, pad.t + ph);
      ctx.stroke();
      ctx.textAlign = index === 0 ? 'left' : index === stations.length - 1 ? 'right' : 'center';
      ctx.fillText(station.name, sx, 4 * dpr + (index % 2) * 9 * dpr);
    });
    ctx.restore();
    ...
    zones.forEach((zone, index) => {
      if (index === 0) ctx.moveTo(x(zone.start), y(zone.limit));
      else ctx.lineTo(x(zone.start), y(zone.limit));
      ctx.lineTo(x(zone.end), y(zone.limit));
    });
    ctx.stroke();
    ctx.setLineDash([]);

    if (samples.length > 1) {
      ctx.strokeStyle = '#5fc3ff';
      ctx.lineWidth = 2.8 * dpr;
      ctx.lineJoin = 'round';
      ctx.lineCap = 'round';
      ctx.beginPath();
      samples.forEach((sample, index) => index ? ctx.lineTo(x(sample.pos), y(sample.speed)) : ctx.moveTo(x(sample.pos), y(sample.speed)));
      ctx.stroke();
    }
    ...
    ctx.beginPath();
    ctx.moveTo(x(pos), pad.t);
    ctx.lineTo(x(pos), pad.t + ph);
    ctx.stroke();

    ctx.fillStyle = 'rgba(230,240,245,.72)';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'top';
    [['0', 0], ['1.4', 1400], ['2.9', 2900], ['4.2km', 4200]].forEach(([label, p]) => ctx.fillText(label, x(p), h - 15 * dpr));
  }

  function tick(now) {
    const pos = routePosition();
    const speed = Number(speedEl.textContent) || 0;
    const limit = Number(limitEl.textContent) || 35;
    ...
    if ((pos - lastSamplePos >= 10 || now - lastSampleTime >= 700) && pos >= lastSamplePos) {
      samples.push({ pos, speed });
      if (samples.length > 800) samples.splice(1, 1);
      lastSamplePos = pos;
      lastSampleTime = now;
    }
    draw();
